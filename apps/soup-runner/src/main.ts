    ...
    <div class="header">
        <div class="header-controls">
            ...
            <div>
                ...
                <a href="/tools" class="refresh-btn" style="text-decoration: none; display: inline-block; margin-left: 0.5rem;">🛠️ Tools</a>
            </div>
        </div>
    </div>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Generated Tools - Agentic Soup</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background: #f5f5f5; }
...
<body>
    <div class="header">
        <h1>🛠️ Generated Tools</h1>
        <p>Top 50 Agent-Created Tools by Success Rate</p>
    </div>

    <div class="nav-links">
        <a href="/dashboard">← Back to Dashboard</a>
        <a href="/responses">Job Responses</a>
        <a href="/leaderboard">Leaderboard</a>
    </div>

    <div class="container">
        <div class="controls">
            <button class="refresh-btn" onclick="loadToolsData()">🔄 Refresh Data</button>
            <div class="info-text">
                Showing tools generated by agents, ordered by success count
            </div>
        </div>
    ...
    <script>
        async function loadToolsData() {
            ...
            try {
                const response = await fetch('/api/tools-table');
                const data = await response.json();
                ...
                data.rows.forEach(tool => {
                    ...
                    successCell.textContent = tool.successCount;

                    const failureCell = row.insertCell();
                    failureCell.textContent = tool.failureCount;

                    const totalCell = row.insertCell();
                    totalCell.textContent = tool.totalAttempts;

                    const rateCell = row.insertCell();
                    rateCell.textContent = tool.successRate;
                    const rate = parseFloat(tool.successRate);
                    if (rate >= 80) {
                        rateCell.className = 'success-rate-high';
                    } else if (rate >= 50) {
                        rateCell.className = 'success-rate-medium';
                    } else {
                        rateCell.className = 'success-rate-low';
                    }

                    const qualityCell = row.insertCell();
                    qualityCell.textContent = tool.avgQuality;

                    const lastUsedCell = row.insertCell();
                    lastUsedCell.textContent = new Date(tool.lastUsed).toLocaleString();

                    const swarmCell = row.insertCell();
                    swarmCell.textContent = tool.swarmName;
                });
            } catch (error) {
                loading.style.display = 'none';
                errorMessage.textContent = 'Error loading tools: ' + error.message;
        ...
        // Auto-refresh every 60 seconds
        setInterval(loadToolsData, 60000);
    </script>
</body>
</html>
`;
...
app.get('/api/system-health', async () => {
  ...
  // Check if running in swarm mode by checking if swarms exist
  const swarms = await prisma.swarm.findMany();
  const isSwarmMode = swarms.length > 0;

  let totalEntities, activeEntities, totalWins, totalAttempts, avgBalance;

  if (isSwarmMode) {
    // Use swarm data
    const activeSwarms = swarms.filter((s: any) => s.alive);
    totalEntities = swarms.length;
    activeEntities = activeSwarms.length;
    totalWins = swarms.reduce((sum: number, s: any) => sum + s.wins, 0);
    totalAttempts = swarms.reduce((sum: number, s: any) => sum + s.attempts, 0);
    avgBalance =
      activeSwarms.length > 0
        ? Math.round(
            activeSwarms.reduce((sum: number, s: any) => sum + s.balance, 0) / activeSwarms.length
          )
        : 0;
  } else {
    ...
    activeEntities = activeAgents.length;
    totalWins = agents.reduce((sum: number, a: any) => sum + a.wins, 0);
    totalAttempts = agents.reduce((sum: number, a: any) => sum + a.attempts, 0);
    avgBalance =
      activeAgents.length > 0
        ? Math.round(
            activeAgents.reduce((sum: number, a: any) => sum + a.balance, 0) / activeAgents.length
          )
        : 0;
  }
  ...
  // Calculate uptime
  const uptimeMs = systemState.startedAt ? Date.now() - systemState.startedAt.getTime() : 0;
  const uptimeHours = Math.floor(uptimeMs / 3600000);
  const uptimeMins = Math.floor((uptimeMs % 3600000) / 60000);
  const uptime = `${uptimeHours}h ${uptimeMins}m`;

  return {
    totalAgents: totalEntities,
    activeAgents: activeEntities,
    totalJobs: jobs,
    totalTransactions,
    recentTransactions,
    avgBalance,
    successRate: totalAttempts > 0 ? Math.round((totalWins / totalAttempts) * 100) + '%' : '0%',
    uptime,
  };
});
...
app.get('/api/metrics/throughput', async () => {
  ...
  // Use 15-minute intervals and fix timestamp comparison
  // ts is stored as milliseconds since epoch (integer), so we need to convert it properly
  const stats = await prisma.$queryRaw`
    SELECT
      datetime((ts/1000 / 900) * 900, 'unixepoch') as interval_time,
      COUNT(*) as count
    FROM Ledger
    WHERE reason = 'payout'
...
// API endpoint for tools table
app.get('/api/tools-table', async () => {
  try {
    const tools = await prisma.agentTool.findMany({
      orderBy: [{ successCount: 'desc' }, { failureCount: 'asc' }],
      take: 50,
    });

    // Get unique swarm IDs from agent IDs
    const swarmIds = [
      ...new Set(
        tools
          .map((tool: any) => {
            // Extract swarm ID from agent ID pattern: swarm_xxx_agent_y -> swarm_xxx
            const match = tool.agentId.match(/^(swarm_\d+_\d+_[a-z0-9]+)_agent_\d+$/);
    ...
    // Fetch swarm names
    const swarms = await prisma.swarm.findMany({
      where: { id: { in: swarmIds as string[] } },
      select: { id: true, name: true },
    });
    const swarmNameMap = new Map(swarms.map((s: any) => [s.id, s.name]));

    const rows = tools.map((tool: any) => {
      const totalAttempts = tool.successCount + tool.failureCount;
      const successRate =
        totalAttempts > 0 ? ((tool.successCount / totalAttempts) * 100).toFixed(1) : '0.0';

      // Extract swarm ID and get name
      const match = tool.agentId.match(/^(swarm_\d+_\d+_[a-z0-9]+)_agent_\d+$/);
      const swarmId = match ? match[1] : null;
      const swarmName = swarmId ? swarmNameMap.get(swarmId) || swarmId : tool.agentId;
      ...
      return {
        ...
        avgQuality: tool.avgQuality.toFixed(1),
    ...
    console.error('[API] Failed to fetch tools table data:', error);
    return {
      rows: [],
      totalRows: 0,
      error: 'Failed to fetch data: ' + (error as Error).message,
