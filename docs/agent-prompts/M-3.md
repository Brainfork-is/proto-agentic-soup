...
Goal: Complete ticket M-3 — Redis setup (local) in this repository with minimal guidance.

## Ticket Context (from docs/tickets.md)

```md
- [ ] **[M-3] Redis (local service)** — S **Desc:** Ensure a single local Redis service on 6379 for queues + pub/sub. **AC:** Local Redis running → health OK; app connects.
```
...
- Create a new branch: feat/m-3-auto, commit changes.
