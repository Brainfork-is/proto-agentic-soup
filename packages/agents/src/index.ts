import { JobData } from '@soup/common'; import { Tools } from './tools';
export class SimpleAgent{
  id:string; temperature:number; tools:string[];
  ...
  async handle(job:JobData){ if(job.category==='web_research'){
    ...
    if(job.category==='classify'){ const {labels,answer}=job.payload as any; return {ok:true,artifact:(labels&&labels[0])||answer||'A'}; }
    if(job.category==='math'){ const {expr}=job.payload as any; const r=await Tools.calc({expr}); return {ok:!!r.ok,artifact:r.ok?String(r.value):'err'}; }
