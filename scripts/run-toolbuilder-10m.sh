...
#   VERTEX_AI_MAX_OUTPUT_TOKENS (use config from .env, no override)
...
# Note: VERTEX_AI_MAX_OUTPUT_TOKENS is now managed by config system, not overridden here
...
# VERTEX_AI_MAX_OUTPUT_TOKENS managed by config system
...
# Note: VERTEX_AI_MAX_OUTPUT_TOKENS uses config-based token limits instead
