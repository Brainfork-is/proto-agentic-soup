...
# Run soup-runner in swarm mode for a fixed duration (default: 10 minutes),
# capture logs and produce a summary + CSV export for later review.

# Usage:
#   bash scripts/run-swarms-10m.sh [minutes]
#
# Environment overrides (optional):
#   JOBS_PER_MIN (default 10)
#   SWARM_COUNT (default 5)
#   AGENTS_PER_SWARM (default 3)
#   LLM_MAX_TOKENS_PER_HOUR (default 5000)
#   LLM_MAX_TOKENS_PER_AGENT (default 150)
#   VERTEX_AI_TEMPERATURE (default 0.7)

MINUTES=${1:-10}
JOBS_PER_MIN=${JOBS_PER_MIN:-10}
SWARM_COUNT=${SWARM_COUNT:-5}
AGENTS_PER_SWARM=${AGENTS_PER_SWARM:-3}
LLM_MAX_TOKENS_PER_HOUR=${LLM_MAX_TOKENS_PER_HOUR:-10000}
LLM_MAX_TOKENS_PER_AGENT=${LLM_MAX_TOKENS_PER_AGENT:-500}
VERTEX_AI_TEMPERATURE=${VERTEX_AI_TEMPERATURE:-0.7}

STAMP=$(date +%Y%m%d-%H%M%S)
RUN_START_ISO=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
RUN_DIR="runs/${STAMP}-swarms"
mkdir -p "$RUN_DIR"
...
echo "[run] Starting soup-runner (swarm mode) for ${MINUTES} minutes..."
echo "[run] Swarms: $SWARM_COUNT x $AGENTS_PER_SWARM agents each"
export USE_SWARMS=1 \
  SWARM_COUNT \
  AGENTS_PER_SWARM \
  JOBS_PER_MIN \
...
# Check leaderboard API for swarm data
echo "[run] Fetching swarm leaderboard..."
LEADERBOARD_FILE="$RUN_DIR_ABS/swarm-leaderboard.json"
if curl -s "http://localhost:3000/leaderboard" >"$LEADERBOARD_FILE" 2>/dev/null; then
  echo "[run] Swarm leaderboard saved: $LEADERBOARD_FILE"
  if command -v jq >/dev/null 2>&1; then
    echo "[run] Top 5 swarms by balance:"
    jq -r '.rows[:5] | .[] | "\(.name): Balance=\(.balance), Wins=\(.wins)/\(.attempts)"' "$LEADERBOARD_FILE" 2>/dev/null || true
  fi
else
  echo "[run] Could not fetch swarm leaderboard from localhost:3000"
fi

# Render markdown table of prompts vs responses (matching tool-builder format)
...
with open("$OUT_CSV", newline='') as f:
    ...
    for row in reader:
        ...
        # Limit prompt length for table readability
        if len(prompt) > 200:
            prompt = prompt[:200] + '...'
...
echo "  - $RUN_DIR/swarm-leaderboard.json"
if [ -d "$RUN_DIR/manifests" ]; then echo "  - $RUN_DIR/manifests/ (snapshot)"; fi
if [ -d "$RUN_DIR/code" ]; then echo "  - $RUN_DIR/code/ (snapshot)"; fi
if [ -f "$RUN_DIR_ABS/final-responses-table.md" ]; then echo "  - $RUN_DIR/final-responses-table.md"; fi